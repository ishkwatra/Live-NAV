...
def estimate_nav_change(portfolio):
    ...
    op1 = f"{errors}"
    op2 = f"{len(portfolio)-errors}/{64} ({(calc_weight*100):.2f}% out of 98.93%) - Rest is Cash, Debt, etc."
...
def run_analysis(depth, investment, units):
    ...
    if raw_lines == []:
        ...
        if investment == 0:
            investment = float(data[1][0])
        if units == 0:
            units = float(data[1][1])
    file.close()
    ...
    if previous_nav == -1:
        return {
            'fund_name': "JM Flexi Cap Fund Direct Growth",
    ...
    print("2. In-Depth Analysis (Slow: 130-140s)")
    str = depth
