...
portfolio2 = {
    ...
    'Poly Medicure Limited': {'symbol': 'POLYMED', 'weight': 0.0060},
...
def estimate_nav_change(portfolio):
    ...
    op1 = f"{errors}"
    op2 = f"{len(portfolio)-errors}/{52} ({(calc_weight*100):.2f}% out of 90.98%) - Rest is Cash, Debt, etc."
...
def run_analysis(depth, investment, units):
    ...
    if raw_lines == []:
        ...
        if investment == 0:
            investment = float(data[4][0])
        if units == 0:
            units = float(data[4][1])
    file.close()
    ...
    if previous_nav == -1:
        return {
            'fund_name': "Trust Small Cap Fund Direct Growth",
